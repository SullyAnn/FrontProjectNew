...
/*const getNextPage = async function(scoreGrade, nextPage) {

    let data;
    const response = await fetch("https://fr.openfoodfacts.org/cgi/search.pl?action=process&tagtype_0=nutrition_grades&tag_contains_0=contains&tag_0="+ scoreGrade + "&page="+ nextPage + "&json=true") 
    if (response.status == 200) {
        data = response.json()
        console.log("getProductByNutriscore request success!")
        console.log(data)
